...
public class EndiannessAwareBinaryReader : BinaryReader
{
    ...
    public float ReadSingle(ByteOrder endianness)
    {
        return BitConverter.ToSingle(ReadForEndianness(sizeof(float), endianness), 0);
    }

    public double ReadDouble(ByteOrder endianness)
    {
        return BitConverter.ToDouble(ReadForEndianness(sizeof(double), endianness), 0);
    }
